...
function addEvent() {
  ...
  for (i of rows) {
    inputNo += 1;
    eventParameter.insertAdjacentHTML(
      "beforeend",
      `<div id='test${inputNo}'><input type='text' name='ep_key' class='form_input' value='${i}'><input type='text' name="ep_value" class='form_input' value='${i}'><i class='remove_button' onclick='deleteInput(${inputNo})'></i></div>`
    );
  }
...
function addUser() {
  ...
  for (i of rows) {
    inputNo += 1;
    userParameter.insertAdjacentHTML(
      "beforeend",
      `<div id='test${inputNo}'><input type='text' name="up_key" class='form_input' value='${i}'><input type='text' name="up_value" class='form_input' value='${i}'><i class='remove_button' onclick='deleteInput(${inputNo})'></i></div>`
    );
  }
...
function addEventInput() {
  ...
  eventParameter.insertAdjacentHTML(
    "beforeend",
    `<div id='test${inputNo}'><input type='text' name="ep_key" class='form_input'><input type='text' name="ep_value" class='form_input'><i class='remove_button' onclick='deleteInput(${inputNo})'></i></div>`
  );
}
...
function addUserInput() {
  ...
  userParameter.insertAdjacentHTML(
    "beforeend",
    `<div id='test${inputNo}'><input type='text' name="up_key" class='form_input'><input type='text' name="up_value" class='form_input'><i class='remove_button' onclick='deleteInput(${inputNo})'></i></div>`
  );
}
...
function changeTagType() {
  ...
  if (checkValue == "gaawc") {
    ...
    document.getElementsByClassName("field_title")[0].innerHTML =
      "이벤트 매개변수";
    document.getElementsByClassName("field_name")[0].innerHTML =
      "매개변수 이름";
...
function changeAAInput() {
  ...
  if (selectUndefined == "none") {
    ...
    formMid.insertAdjacentHTML(
      "beforeend",
      `<div class="caption">측정 ID</div><input type="text" class="form_input" id="measurementId">`
    );
  } else {
    formMid.replaceChildren();
    formMid.insertAdjacentHTML(
...
function setData() {
  if (validation()) {
    ...
    let setTag = {
      ...
      triggerType: document.querySelector('input[name="triggerType"]:checked')
        .value,
      triggerId: triggerId,
      VAR_Array: eventArr,
      user_Array: userArr,
    ...
    //트리거 설정
    const triggerType = document.querySelector(
      'input[name="triggerType"]:checked'
    ).value;
...
function setDataList() {
  ...
  tags.forEach((e) => {
    ...
    const iconClass =
      e.triggerType == "pageview" ? "list_icon pageview" : "list_icon event";
...
function deleteTag() {
  ...
  Array.from(checked).forEach((checkbox) => {
    ...
    var triggerName =
      checkbox.parentElement.parentElement.lastElementChild.innerText;
  ...
  for (q of tags) {
    if (e.innerText === q.tagName) {
      ...
      for (j of q.VAR_Array) {
        ...
        eventParameter.insertAdjacentHTML(
          "beforeend",
          `<div id='test${inputNo}'><input type='text' name="ep_key" class='form_input' value="${j.name}"><input type='text' name="ep_value" class='form_input' value="${j.variable}"><i class='remove_button' onclick='deleteInput(${inputNo})'></i></div>`
        );
      }
      for (z of q.user_Array) {
        ...
        userParameter.insertAdjacentHTML(
          "beforeend",
          `<div id='test${inputNo}'><input type='text' name="up_key" class='form_input' value="${z.name}"><input type='text' name="up_value" class='form_input' value="${z.variable}"><i class='remove_button' onclick='deleteInput(${inputNo})'></i></div>`
        );
      }
      //button class명 추가
      document
    ...
    for (i in tags) {
      ...
      if (tags[i].tagName == updateTagName) {
        if (triggers.length == 0) {
          ...
          for (j in triggers) {
            if (
              tags[i].triggerId == triggers[j].triggerId &&
              tags[i].triggerType != "pageview"
            ) {
              if (!Object.keys(setTrigger).length == 0) {
...
function validation(type) {
  ...
  if (type != "update") {
    ...
    // 업데이트 시 태그 이름 중복 검사: 현재 선택한 태그 이름과 다를 시에 중복 검사 실시
  } else {
  ...
  if (triggerName) {
    ...
    if (type != "update") {
      ...
      // 업데이트 시 태그 이름 중복 검사: 현재 선택한 트리거 이름과 다를 시에 중복 검사 실시
    } else {
      if (
        updateTriggerName != triggerName.value &&
        updateTriggerName != "All Pages"
      ) {
        for (i of triggers) {
          if (i.name == triggerName.value) {
            openDialog("dupValue", "트리거");
  ...
  switch (errorType) {
    case "noSave":
      errorMsg = {
        title: "저장되지 않은 변경사항",
        content:
          "이 페이지를 닫으시겠습니까? 변경사항을 저장하지 않았습니다. 페이지를 닫으면 변경사항이 손실됩니다.",
      };
      break;
    case "noValue":
      errorMsg = {
        ...
        content:
          "이 태그에 " +
          value +
          " 값이 없습니다. 해당 값을 입력하지 않으면 태그를 저장할 수 없습니다.",
      };
      break;
    case "dupValue":
      errorMsg = {
...
function errorLabel(type, e) {
  switch (type) {
    case "noInput":
      if (e.parentElement.lastChild.className != "errorLabel") {
        e.parentElement.insertAdjacentHTML(
          "beforeend",
          `<div class="errorLabel">값을 입력해야 합니다.</div>`
        );
      }
      break;
    case "noSelectTag":
    ...
    case "noRegex":
      ...
      if (
        measurementId &&
        measurementId.parentElement.lastChild.className != "errorLabel"
      ) {
        document
          .getElementById("form_measurementId")
          .insertAdjacentHTML(
            "beforeend",
            `<div class="errorLabel">올바른 측정 ID를 입력하세요(예: G-1234)</div>`
          );
      }
      break;
    case "noEco":
      if (e.parentElement.lastChild.className != "errorLabel") {
        e.parentElement.insertAdjacentHTML(
          "beforeend",
          `<div class="errorLabel">구성 태그에는 전자상거래 설정이 불가합니다.</div>`
        );
      }
      break;
  }
